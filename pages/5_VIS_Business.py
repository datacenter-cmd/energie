...
if "df_edit_bus" not in st.session_state or st.session_state.get("mese_bus") != mese_sel:
    st.session_state.df_edit_bus = df.copy()
    st.session_state.mese_bus = mese_sel

# ── Normalizza tipi ───────────────────────────────────────────────────────────
def normalizza(df_in):
    df_out = df_in.copy()
    for _c in ["negozio", "operatore", "ragione sociale", "piano tariffario inserito",
        ...
        if _c in df_out.columns:
            df_out[_c] = df_out[_c].fillna("").astype(str).str.strip().replace("nan", "")
    if "id pratica" in df_out.columns:
        def to_int_str(v):
            try:
                if str(v).strip() in ("", "nan"): return ""
                return str(int(float(str(v).strip())))
            except: return str(v).strip()
        df_out["id pratica"] = df_out["id pratica"].apply(to_int_str)
    if "partita iva" in df_out.columns:
        def to_piva_str(v):
            try:
                if str(v).strip() in ("", "nan"): return ""
                return str(int(float(str(v).strip()))).zfill(11)
            except: return str(v).strip()
        df_out["partita iva"] = df_out["partita iva"].apply(to_piva_str)
    if "seriale sim" in df_out.columns:
        def to_sim_str(v):
            try:
                if str(v).strip() in ("", "nan"): return ""
                return f"{int(float(str(v).strip()))}"
            except: return str(v).strip()
        df_out["seriale sim"] = df_out["seriale sim"].apply(to_sim_str)
    return df_out

df_work = normalizza(st.session_state.df_edit_bus)
...
if vista == "🖥️ Desktop (tabella)":
    if st.button("➕ Aggiungi pratica"):
        ...
        st.session_state.df_edit_bus = pd.concat([df_work, pd.DataFrame([new_row])], ignore_index=True)
        st.rerun()
    ...
    edited = st.data_editor(
        ...
        key=f"editor_bus_{mese_sel}"
    )
    st.session_state.df_edit_bus = edited
...
# ── MOBILE ────────────────────────────────────────────────────────────────────
    ...
    for i, row in df_work.iterrows():
        with st.expander(f"#{i+1} — {row.get('ragione sociale','') or 'Nuova pratica'} | {row.get('negozio','')}", expanded=False):
            ...
            df_work.at[i, "piano tariffario inserito"] = c1.selectbox("Piano tariffario", [""] + PIANI, key=f"ptt_{i}")
    ...
    st.session_state.df_edit_bus = df_work
...
with col_s1:
    if st.button("💾 Salva", use_container_width=True):
        ...
        for m in MESI:
            if m == mese_sel:
                all_data[m] = st.session_state.df_edit_bus
            else:
...
with col_s2:
    buf = io.BytesIO()
    st.session_state.df_edit_bus.to_excel(buf, index=False)
    st.download_button(
        "⬇️ Esporta Excel",
        data=buf.getvalue(),
